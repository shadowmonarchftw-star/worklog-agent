...
function serverUrl(baseUrl, path) {
  return `${String(baseUrl).trim().replace(/\/+$/, "")}/${path}`;
}

function completionsUrl(baseUrl) {
  return serverUrl(baseUrl, "chat/completions");
}
...
// Typing an exact model tag is the easiest way to misconfigure a local server,
// so the list is offered as a suggestion. Servers that do not implement the
// endpoint return nothing rather than an error, because the typed name still
// works — only the convenience is missing.
export async function listLocalModels({
  ...
  let response;
  try {
    response = await fetchImpl(serverUrl(baseUrl, "models"), {
      signal: combined,
      // A redirect must not be able to move an authenticated probe to a host the
