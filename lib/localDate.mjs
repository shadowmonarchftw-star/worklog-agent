...
// Walks calendar days in UTC so the result never shifts with the host timezone
// or a DST transition. Inputs and outputs are YYYY-MM-DD strings.
export function enumerateDates(start, end) {
  ...
  for (let value = start; value <= end; value = cursor.toISOString().slice(0, 10)) {
    dates.push(value);
