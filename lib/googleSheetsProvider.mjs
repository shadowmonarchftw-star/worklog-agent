...
export function buildSheetWriteData({ tab, rowNumber, includeDate, row }) {
  return [
    ...(includeDate ? [{
      range: formatSheetRange(tab, `A${rowNumber}`),
      values: [[row.date]],
    }] : []),
    {
      range: formatSheetRange(tab, `B${rowNumber}`),
      values: [[row.summary]],
    },
    {
      range: formatSheetRange(tab, `D${rowNumber}`),
      values: [[row.hours]],
    },
  ];
}
...
async function context(input) {
  ...
  return { spreadsheetId, tab, token, values: data.values || [], fetchImpl };
}
