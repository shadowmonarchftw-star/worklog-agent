...
function validateSettings(settings) {
  ...
  if (settings.skipDate !== undefined && settings.skipDate !== null && !/^\d{4}-\d{2}-\d{2}$/.test(settings.skipDate)) {
...
function requireAttemptLease(
  ...
  if (!allowedStatuses.includes(row.status)) {
    throw new Error(
      `Automation operation requires attempt status ${allowedStatuses.join(" or ")}`,
    );
  }
  const lease = db.prepare(
    `SELECT 1 FROM automation_lease
     WHERE singleton = 1
       AND owner_id = ?
       AND attempt_id = ?
       AND expires_at > ?`,
  ).get(ownerId, attemptId, now);
  if (!lease) {
    throw new Error(
      "Automation operation requires a matching unexpired lease",
    );
  }
  return row;
}
...
export function claimAutomationAttempt(
  ...
  const claim = db.transaction(() => {
    ...
    // A day closed by no_activity produced no sheet write and no worklog, so
    // it is not finished. Only success and an exhausted failure close it.
    if (
      trigger === "automatic" &&
      day.terminal_outcome &&
      day.terminal_outcome !== "no_activity"
    ) {
    ...
    if (trigger === "automatic") {
      ...
      // no_activity attempts wrote nothing, so they do not spend the retry
      // budget that exists to bound repeated failures.
      const spent = prior.filter(({ status }) => status !== "no_activity");
      if (
        spent.length >= 2 ||
        (spent.length === 1 &&
          !["failed", "interrupted"].includes(spent[0].status))
      ) {
        return { outcome: "retry_limit", attempt: null };
      }
      retryOfId = spent[0]?.id || prior[0]?.id || null;
    }
...
export function claimAutomationRecovery(
  db,
  {
    attemptId,
    ownerId,
  ...
  const claim = db.transaction(() => {
    expireLeaseOwner(db, claimedAt);
    const attempt = fetchAttempt(db, attemptId);
    if (!attempt) throw new Error("Automation attempt not found");
    if (attempt.status !== "interrupted") {
      throw new Error("Only interrupted automation attempts can be recovered");
    }
    const day = db
      .prepare("SELECT terminal_outcome FROM automation_days WHERE id = ?")
      .get(attempt.day_id);
    if (day?.terminal_outcome) {
      throw new Error("Automation recovery rejected because the day is terminal");
    }
    const superseded = db.prepare(
      `SELECT 1 FROM automation_attempts
       WHERE day_id = ? AND id != ? AND (
         retry_of_id = ?
         OR created_at > ?
    ...
    if (superseded) {
      throw new Error(
        "Automation recovery rejected because the attempt was superseded",
      );
    }

    const lease = db.prepare(
      `SELECT owner_id, attempt_id, expires_at FROM automation_lease
       WHERE singleton = 1`,
    ).get();
    if (lease.attempt_id && lease.expires_at > claimedAt) {
      if (lease.attempt_id === attemptId && lease.owner_id === ownerId) {
        return { outcome: "claimed", attempt: mapAttempt(attempt) };
      }
      return { outcome: "already_running", attempt: null };
    }
    db.prepare(
      `UPDATE automation_lease
       SET owner_id = ?, attempt_id = ?, expires_at = ?
       WHERE singleton = 1`,
    ).run(ownerId, attemptId, leaseExpiry(claimedAt));
    return { outcome: "claimed", attempt: mapAttempt(attempt) };
  });
  return claim.immediate();
}
...
export function checkpointAutomationIntent(
  ...
  const checkpoint = db.transaction(() => {
    ...
    const managedRow = normalizeIntendedRow(intendedRow);
    const intendedRowHash = createHash("sha256")
      .update(JSON.stringify({
        date: managedRow.date,
        summary: managedRow.summary,
        hours: managedRow.hours,
      }))
      .digest("hex");
    if (
...
export function transitionAutomationAttempt(
  ...
  const transition = db.transaction(() => {
    ...
    const retryEligible = row.trigger === "automatic" &&
      automaticAttemptCount === 1;
    const nextRetryDueAt = retryDueAt && (to !== "failed" || retryEligible)
      ? iso(retryDueAt, "retryDueAt")
      : null;
    ...
    if (closesDay) {
      // no_activity is provisional: a later run on the same day may still
      // produce a real worklog, and that outcome must replace it.
      const finalizableOutcome = to === "failed"
        ? "terminal_outcome IS NULL"
        : "(terminal_outcome IS NULL OR terminal_outcome IN ('failed', 'no_activity'))";
      db.prepare(
        `UPDATE automation_days SET
          ...
          updated_at = ?,
          completed_at = ?
         WHERE id = ? AND ${finalizableOutcome}`,
      ).run(
        to,
...
export function getAutomationStatus(
  ...
  const automaticError = latestAttempt(
    db,
    "trigger = 'automatic' AND status = 'failed'",
    true,
  );
  const automaticSuccess = latestAttempt(
    db,
    "trigger = 'automatic' AND status = 'success'",
    true,
  );
  // An error a later automatic run already recovered from is history, not a
  // condition the user still has to act on.
  const resolved = Boolean(
    automaticError?.completedAt &&
      automaticSuccess?.completedAt &&
      automaticSuccess.completedAt > automaticError.completedAt,
  );
  return {
    ...
    lastAutomaticAttempt: latestAttempt(db, "trigger = 'automatic'"),
    lastManualAttempt: latestAttempt(db, "trigger = 'manual'"),
    lastSuccess: latestAttempt(db, "status = 'success'", true),
    lastError: latestAttempt(db, "status = 'failed'", true),
    lastAutomaticError: resolved ? null : automaticError,
  };
}
